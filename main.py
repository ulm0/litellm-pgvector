...
async def create_vector_store(
    ...
    try:
        ...
        expires_at = (
            int(datetime.fromisoformat(str(vector_store["expires_at"])).timestamp())
            if vector_store.get("expires_at")
            else None
        )
        last_active_at = (
            int(datetime.fromisoformat(str(vector_store["last_active_at"])).timestamp())
...
async def list_vector_stores(
    ...
    try:
        ...
        for row in results:
            ...
            expires_at = (
                int(datetime.fromisoformat(str(row["expires_at"])).timestamp())
                if row.get("expires_at")
                else None
            )
            last_active_at = (
                int(datetime.fromisoformat(str(row["last_active_at"])).timestamp())
...
async def delete_vector_store(
    vector_store_id: str, api_key: str = Depends(get_api_key)
):
    try:
        vector_store_table = settings.table_names["vector_stores"]
        result = await db.query_raw(
...
@app.delete(
    ...
    response_model=EmbeddingDeleteByMetadataResponse,
)
...
@app.post(
    "/v1/vector_stores/{vector_store_id}/embeddings/delete",
    response_model=EmbeddingDeleteByMetadataResponse,
)
async def delete_embeddings_by_metadata_post(
    vector_store_id: str,
    request: EmbeddingDeleteByMetadataRequest,
    api_key: str = Depends(get_api_key),
):
    return await delete_embeddings_by_metadata(vector_store_id, request, api_key)
